    ...
    {/* Navbar */}
          <nav className="h-[70px] sm:h-[80px] flex items-center justify-between px-4 sm:px-6 lg:px-12 bg-[#fffdf7]/95 backdrop-blur shadow-sm sticky top-0 z-50">
            ...
            <div className="hidden md:flex gap-8">
              <Link to={"/"} className="cursor-pointer hover:text-[#e11d48]">Home</Link>
              <Link to={"/menu"} className="cursor-pointer hover:text-[#e11d48]">Menu</Link>
              <Link to={"/gifting"} className="cursor-pointer hover:text-[#e11d48]">Gifting</Link>
              <Link to={"/about-us"} className="cursor-pointer hover:text-[#e11d48]">About Us</Link>
            </div>
    
            <div className="flex items-center gap-2 sm:gap-4">
              <Link to={'/cart'} className="w-9 h-9 sm:w-10 sm:h-10 rounded-full border flex items-center justify-center hover:bg-[#fff1f2]">
                <FiShoppingBag />
              </Link>
